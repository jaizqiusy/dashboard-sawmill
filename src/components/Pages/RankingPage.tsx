...
export function RankingPage({ data }) {
  ...
  return (
    <div className="p-5 space-y-6">
      <div className="bg-[#1e293b]/60 backdrop-blur-md rounded-2xl p-5 border border-white/5 shadow-lg relative overflow-hidden">
        <div className="absolute top-0 right-0 w-32 h-32 bg-amber-500/10 rounded-full blur-3xl -mr-16 -mt-16" />
        <h2 className="text-lg font-bold text-white flex items-center gap-2 tracking-tight relative z-10">
          <Trophy className="w-5 h-5 text-amber-400" />
          Leaderboard
        </h2>
        <p className="text-[11px] text-slate-400 mt-1 uppercase tracking-widest font-medium relative z-10">Ranking Performa Mesin</p>

        <div className="flex gap-2 mt-4 relative z-10">
          <div className="flex bg-[#0f172a]/50 border border-white/10 rounded-lg p-1">
             <button 
          ...
          <select 
            className="bg-[#0f172a]/50 border border-white/10 text-slate-300 text-xs font-medium rounded-lg px-3 py-1 outline-none"
            value={periodValue}
        ...
        {rankings.map((rank, i) => {
          ...
          return (
            <div key={i} className={cn(
              ...
              isTop3 ? bgColors[i] : "bg-[#1e293b]/40 border-white/5"
            )}>
              <div className="flex items-center gap-4">
                <div className={cn(
                  "w-8 h-8 rounded-full flex items-center justify-center font-black text-sm",
                  isTop3 ? textColors[i] + ' bg-black/20' : "text-slate-500 bg-black/20"
                )}>
                  {i + 1}
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-[10px] text-slate-400 uppercase tracking-wider font-medium">Vol: {rank.utama.toFixed(1)} m³</span>
                  </div>
                </div>
              </div>
              <div className="text-right">
                <span className="text-[9px] text-slate-500 uppercase tracking-widest block mb-0.5">Efficiency</span>
                <span className={cn(
