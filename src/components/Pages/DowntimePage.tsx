...
export function DowntimePage({ data }) {
  ...
  return (
    <div className="p-5 space-y-6">
      <div className="bg-[#1e293b] rounded-2xl p-5 border border-rose-500/20 shadow-lg relative overflow-hidden">
        ...
        <p className="text-[11px] text-slate-300 mt-1 uppercase tracking-widest font-bold relative z-10">Monitoring kerusakan & perbaikan mesin</p>

        <div className="flex gap-2 mt-4 relative z-10 overflow-x-auto no-scrollbar pb-1">
          <div className="flex bg-[#0f172a] border border-white/15 rounded-lg p-1 shrink-0">
          ...
          <select 
            className="bg-[#0f172a] border border-white/15 text-slate-200 text-xs font-bold rounded-lg px-3 py-1 outline-none shrink-0"
            value={periodValue}
      ...
      <div className="space-y-4">
        {machines.map(mName => {
          ...
          return (
            <div key={mName} className="bg-[#1e293b] rounded-2xl border border-rose-500/20 shadow-lg overflow-hidden relative">
              <div className="absolute left-0 top-0 bottom-0 w-1 bg-rose-500/50" />
              <div className="p-4 flex items-center justify-between border-b border-white/10 bg-rose-500/10">
                <h3 className="text-white font-bold ml-2">{mName}</h3>
                <span className="px-2 py-0.5 bg-rose-500/30 text-rose-300 text-[10px] font-black uppercase tracking-wider rounded border border-rose-500/30">
                  {rows.length} Events
                </span>
              </div>
                  ...
                  return (
                    <div key={idx} className="bg-black/30 rounded-xl p-3 border border-white/5">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-[10px] text-slate-300 font-bold uppercase tracking-widest">{row.tanggal}</span>
                        {rawTotalMins > 0 && <span className="text-[10px] text-rose-400 font-mono font-black bg-rose-500/20 px-1.5 py-0.5 rounded">{rawTotalMins} mnt</span>}
                      </div>
                      <div className="flex flex-col gap-1.5">
                        {parts.map((p, i) => (
                          <span key={i} className="text-xs text-slate-200 font-medium flex items-start gap-1.5">
